...
def load_production_models():
    print("Loading production components (LightGBM)...") # ⬅️ Sửa log
    ...
    if not all(os.path.exists(p) for p in [pipeline_path, scaler_path]):
        raise FileNotFoundError("pipeline/scaler not found. Please run lightgbm.py first.")
