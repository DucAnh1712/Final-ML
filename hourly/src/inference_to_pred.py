...
def load_production_models():
    ...
    if not all(os.path.exists(p) for p in [pipeline_path, scaler_path]):
        raise FileNotFoundError("pipeline/scaler not found. Please run train_lightgbm.py first.")
    ...
    for target_name in config.TARGET_FORECAST_COLS:
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Model {model_name} not found. Please run train_lightgbm.py.")
...
def main():
    ...
    print(f"\n--- Evaluating {len(config.TARGET_FORECAST_COLS)} horizons (LightGBM) ---")
