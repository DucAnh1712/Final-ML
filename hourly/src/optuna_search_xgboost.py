...
import yaml
import optuna
import xgboost as xgb # ⬅️ CHANGE: Switched to XGBoost
...
def xgboost_objective(trial, X_all_train_raw, y_all_train_raw):
    """
    Optuna objective function for XGBoost (with Early Stopping and Purged CV).
    """
    ...
    for fold_num, (train_idx, val_idx) in enumerate(tscv.split(X_all_train_raw)):
        ...
        # 7. Train XGBoost model with early stopping
    ...
    # Remove early stopping rounds before storing parameters
...
# =============================================================================
def run_optuna_search_xgboost():
    """
    Main function to orchestrate the Optuna search for XGBoost models 
    across all specified target columns.
    """
    task = Task.init(
        project_name=config.CLEARML_PROJECT_NAME,
        task_name="Optuna XGBoost (Hourly)",
        tags=["Optuna", "XGBoost", "Multi-Horizon", "Purged-CV", "Hourly"]
    )
    ...
    for target_name in config.TARGET_FORECAST_COLS:
        ...
        study.optimize(
            lambda trial: xgboost_objective(trial, X_all_train_raw, y_all_train_raw), 
        ...
        # Print results
        print(f"\n🏆 BEST XGBOOST RESULTS FOR {target_name}:")
    ...
    output = {
        ...
        "config": {
            ...
            "n_splits": config.CV_N_SPLITS,
            "gap_rows": config.CV_GAP_ROWS,
            "search_space": config.XGBOOST_PARAM_RANGES,
            "leakage_safe": True
        }
    }
